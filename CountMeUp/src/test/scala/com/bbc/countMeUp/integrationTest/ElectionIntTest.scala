...
class ElectionIntTest extends FunSpec with Matchers {
  ...
  describe("Election round trip test"){
    it("I should be able to add a election, and get its results with no data loss"){
      ...
      readElection.results.map(r => r.candidate) should contain(candidate1)
      readElection.results.map(r => r.candidate) should contain(candidate2)
      readElection.results.map(r => r.candidate) should contain(candidate3)
      readElection.results.size should equal(3)
    }
  }
